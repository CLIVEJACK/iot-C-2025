...
## 5일차
- 함수 포인터 [C언어](./Day5/voidpointer.c)
    - 함수 이름자체가 포인터다 
    - 함수 선언, 함수 정의, 함수 호출
- 보이드 포인터 [C언어](./Day5/voidpointer2.c) 
    - void 타입이면 출력함수에 자료형을 명시해줘야한다.
        - `printf("*p: %d\n", *p);` 이게 아니라 `printf("*p: %d\n", *(int *)p);` 이런식으로 넣어야한다
- 메모리 동적할당 [C언어](./Day/mallo.c)
    - calloc - 메모리 동적 할당하여 0으로 초기화
    - realloc - 크기조절


### C++
- 구조체 선언 [C언어](./Day5/struct.cpp)
    
       ```C++
        struct 구조체명 {				//구조체 선언 struct 구조체명
	    char name[100];                 // 멤버변수 
	    int age;                        // 멤버변수
        };
        ...
        int main()
        {
	    struct 구조체명 변수;             // 구조체 선언

	    return 0;

        }
        ```       
    - 구조체안에 구조체 가능 [C언어](./Day5/struct3.cpp)
    - typedef은 기본자료형을 다른 형채로 선언해서 쓰겠다 [C언어](./Day5/struct4.c)
    - 구조체 포인터[C언어](./Day5/stpoint.cpp)
        - 구조체 변수를 통해서 멤버에 접근하는 방법은 '.'(멤버접근 연산자)사용
        - 구조체 포인터를 통해서 맴버에 접근하는 방법은 '->'(화살표 연산자)사용
    - 구조체 배열 
        - 구조체에서 여러 형태의자료형을 선언한후에 배열을 만듬
        - 파이썬에 리스트랑 비슷한구조
